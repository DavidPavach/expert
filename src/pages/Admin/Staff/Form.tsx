...
import { Lock, Sms, TagUser } from "iconsax-reactjs";
import { Loader } from "lucide-react";
import { useState } from "react";
import { toast } from "react-fox-toast";
import FormField, { InputBase } from "#/components/FormField";
import { cn } from "#/lib/utils";
import { useNewAdmin } from "#/services/mutations.service";
				...
				<form className="gap-5 grid grid-cols-1 px-6 md:px-8 py-7">
						...
						<InputBase
							icon={
								<Sms
									variant="Bold"
									className={`size-4.5 ${
										formData.email.trim() ? "text-primary" : ""
									}`}
								/>
							}
							type="email"
					...
					>
						<button
							...
							onClick={() =>
								update(
									"role",
									formData.role === "ADMIN" ? "SUPER_ADMIN" : "ADMIN",
								)
							}
					...
					>
						...
						{newAdmin.isPending ? (
							...
							<TagUser className="inline ml-0.5 size-5" />
						)}
					</button>
				</form>
			</section>
