...
import { useResetPassword } from "@/services/mutations.service";
...
const ResetForm = () => {
	...
	const handleReset = () => {
		...
		reset.mutate(
			{ email: email.toLowerCase(), password: newPassword },
			{
				onSuccess: () => {
				...
				onError: () => {
					toast.error(
						"Failed to reset password, kindly try again or restart the process.",
					);
					setNewPassword("");
				},
			},
	...
	return (
		...
		>
			<div className="space-y-2">
				...
				<div className="relative">
					<Input
						...
						onBlur={() => isStrongPassword(newPassword)}
						className="border-border text-foreground placeholder:text-muted-foreground montserrat"
						disabled={reset.isPending}
					/>
					<button
						type="button"
						className="top-1/2 right-4 absolute -translate-y-1/2 cursor-pointer"
			...
			<Button
				onClick={handleReset}
				disabled={reset.isPending || errors.trim().length > 0}
				className="bg-primary hover:bg-accent py-3 w-full font-semibold text-primary-foreground uppercase duration-300 hover:text-accent-foreground montserrat"
			>
				{reset.isPending ? (
