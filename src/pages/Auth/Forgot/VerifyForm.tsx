...
import { useVerifyPasswordReset } from "@/services/mutations.service";
...
const VerifyForm = () => {
	...
	const verify = useVerifyPasswordReset();
	const handleVerification = () => {
		...
		if (value.length !== 6)
			return toast.error(
				"Kindly enter the sent Six (6) Digit for Verification ",
			);

		verify.mutate(
			{ email: email.toLowerCase(), otp: value },
			{
				onSuccess: () => {
