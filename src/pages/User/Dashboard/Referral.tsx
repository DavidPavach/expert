...
export default function ReferralBox() {
	...
	const referralLink = `https://expertmirrorcon.com/create?ref=${user?.username}`;
		...
		try {
			if (navigator.share) {
				await navigator.share({
					title: "Join Expertmirrorcon",
					text: "Sign up with my referral link and get started on Expertmirrorcon.",
					url: referralLink,
				});
				setShared(true);
	...
	return (
		<div className="bg-card/85 shadow-sm backdrop-blur my-5 p-4 md:p-5 xl:p-6 border border-border/70 rounded-2xl">
			<div className="flex flex-col gap-5">
				<div className="flex items-start gap-4">
					...
					<div className="min-w-0">
						...
						<p className="mt-1 text-[11px] text-muted-foreground md:text-xs xl:text-sm leading-5">
							...
							link. When they sign up and qualify, you can earn up to 5% in
							referral bonus.
						</p>
					</div>
				</div>
				...
				<div className="gap-4 grid md:grid-cols-[160px_minmax(0,1fr)]">
					<div className="bg-background/70 px-4 py-3 border border-border/70 rounded-2xl">
						...
						<p className="mt-2 font-heading font-semibold text-[11px] text-foreground md:text-xs xl:text-sm truncate capitalize">
							{user?.username}
						</p>
					</div>
					...
					<div className="bg-background/70 p-2 border border-border/70 rounded-2xl">
						<div className="flex md:flex-row flex-col gap-2">
							...
							<button
								type="button"
								onClick={handleShare}
								className="inline-flex justify-center items-center gap-2 bg-card hover:bg-muted px-4 border border-border rounded-xl h-11 font-semibold text-[11px] text-foreground md:text-xs xl:text-sm transition-colors"
							>
								{shared ? (
									<>
